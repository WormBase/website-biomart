...
$CONFIG = {
  "TBL_${dataset}__${acedb_class}__main" => {
    ...
    &insert_simple_dimension('Isolation.Transposon_excision',$dataset,
                             'transposon_family'),
    ...
    # Description.Silent
    #TODO: dmlist/dminfo
    "TBL_${dataset}__sense__dm" => {
      OBJECTS => sub{
        my @objs;
        foreach my $tag( $_[0]->at('Description.Nonsense') ){
          # TODO: add nonsense type (Amber_UAG etc) once tag populated
          push @objs, { sense    => 'Nonsense',
                        text => $tag->right ? $tag->right->name : undef };
        }
        foreach my $mistxt( &names_at($_[0],'Description.Missense') ){
          push @objs, {sense => 'Missense', text => $mistxt}
        }
        foreach my $siltxt( &names_at($_[0],'Description.Silent') ){
          push @objs, {sense => 'Silent', text => $siltxt}          
        }
        return @objs;
      },
      VAL_sense    => sub{ ref($_[0]) eq 'HASH' ? $_[0]->{sense} : undef},
      #VAL_nonsense => sub{ ref($_[0]) eq 'HASH' ? $_[0]->{nonsense} : undef},
      VAL_text     => sub{ ref($_[0]) eq 'HASH' ? $_[0]->{text} : undef },
    },


    #--------------------
    # Description.Splice_site
    &insert_val('Text','Description.Splice_site', 'splice_site'),    
    VAL_splice_site_text => sub{
      my @splice_site;
      my $root = "Description.Splice_site";
      if( my $donor_val = &at($_[0],"$root.Donor" ) ){
        push( @splice_site, "[Donor] $donor_val" );
      }
      if( my $accept_val = &at($_[0],"$root.Acceptor" ) ){
        push( @splice_site, "[Acceptor] $accept_val" );
      }
      return join( ' | ', @splice_site );
    },


    #--------------------
    # Description.Frameshift
    VAL_frameshift => sub{ &dmlist_at($_[0],'Description.Frameshift') },

    #--------------------
    # Description.Maternal,Paternal
    # Vanished from schema (WS146)!    
    #VAL_parentage => sub{
    #  my @parentage;
    #  push( @parentage, &names_at($_[0],'Description.Maternal') );
    #  push( @parentage, $_[0]->at('Description.Paternal') );
    #},

...
##$ace_user = 'ruihua';
$ace_pass = 'wpd0phw3';
$mart_dbname = 'wormmart_220';
##$mart_dbname = 'wormmart_220_dev';
$mart_host = '127.0.0.1';
$mart_port = '3306';
$mart_user = 'example';
$mart_pass = '';
...
#############################################################
    ...
    ##$mart_user   ||= $CONNECTION->{MART_USER} ||= 'example';
    ##$mart_pass   ||= $CONNECTION->{MART_PASS} ||= '';
    ...
    $ACEDB = Ace->connect(-host=>$ace_host,-port=>$ace_port,-user=>$ace_user,-pass=>$ace_pass,
                          #-cache=>{} 
                          )
       || die( "[*DIE] AceDB Connection failure ".
                "$ace_user\@$ace_host:$ace_port: ",
...
#           _test_print($data);
	  ...
	  my $c = 0;
	    ...
	    $c++;
	    print "ace2mart_L351_c_$c==obj=$obj\n";
...
####test by RF_1210#################
		  ##my $obj_test = $obj->name;
		  ##print "ace2mart_L364_obj_test=$obj_test\n";
#########################################
		  $create = 1;
	      }
	      next unless $create;
	    ...
	    ####test by RF_011211
	    ##print "ace2mart_main_L371_before_calling_fetch_data\n";
###########################################
          my $data = &fetch_data( $top_table, $config, $obj );
	    ####test by RF_011211
	    ##print "ace2mart_main_L375_after_calling_fetch_data\n";
###########################################
          &write_data( $mart_dbh, $data );
          $config->{'DESTROY'} && &{$config->{'DESTROY'}}($obj); # cleanup 
          &examine_cache($objects_ref);
...
sub fetch_data{
  my $table  = shift;
####test by RF_012011
  ##print "ace2mart_inside_fetch_data_L426_table=$table\n";
######################################
  my $config = shift;
  my $parent_obj = shift;
...
#  };
  ...
  foreach my $stub_obj( @self_objects ){ # 1 table row per object
	####test by RF_1210#######################
     ##print "ace2mart_L458_stub_obj==$stub_obj\n";
##########################################
    $TABLE_ROWS{$table} ++;
#     $TABLE_ROWS{$table} = $mart_dbh->
    ...
    my $obj_dm_data = {};
####test by RF_012011
      ##print "ace2mart_inside_fetch_data__L470_obj=$obj\n";
##############################################
    foreach my $attrib( keys %$config ){
	####test by RF_1210#######################
	   ##print "ace2mart_inside_sub_fetch_data_L474_attrib=$attrib\n";
##########################################
...
####test by RF_1210#################
	...
	##print "ace2mart_inside_sub_fetch_data_before_recursive_call_L485_bbbb==obj=$obj==parent_obj=$parent_obj==pparent_obj=$pparent_obj==attrib=$attrib==retriever=$retriever\n";
...
####test by RF_1210#################
	...
	##print "ace2mart_inside_sub_fetch_data_after_recursive_call_L492_aaaa_attrib=$attrib==data=$data\n";
...
####test by RF_1210#################
	...
	##print "ace2mart_inside_sub_fetch_data_L495_tttt\n";
...
####test by RF_011211
      ##print "ace2mart_inside_create_mart_table_L593=tbl=$tbl\n";
...
####test by RF_012011
      ...
      ##print "print ace2mart_inside_fetch_schema_L714=tbl=$tbl\n";  ##SequenceInfo_PCRProduct__dm existed here
###################################
...
sub write_row_data{
  ...
  foreach my $f( keys %data ){
    my $v = $data{$f} || next;
####test by RF_011211
    ##if(($table_name =~ /ortholog__dm/) || ($table_name =~ /paralog__dm/)){
      ##print "ace2mart_L824_inside_write_row_data_table=$table_name==k=$f==v=$v\n";
    ##}
###################################################
...
sub create_table{
  ...
  my $table_name = shift;
####test by RF_012011
  ##print "ace2mart_L850_inside_create_table=table_name=$table_name\n";
#################################
  my %columns    = %{shift @_};

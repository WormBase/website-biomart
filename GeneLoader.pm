...
$CONFIG = {
  "TBL_${dataset}__gene__main" => {
    ...
    VAL_species         => sub{ &names_at($_[0],'Identity.Species') },
    ...
    while( $root ){
      ...
      if( ( $start and $end ) and
          ...
          ( $gene_name =~ /(^\w+\.\d+)(.*)/ or
            ...
            $gene_name =~ /(C[A-Z]{2}\d+)/ or # Briggsae (CBG) Remanii (CRE) 
